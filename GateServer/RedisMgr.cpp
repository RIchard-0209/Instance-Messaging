...
RedisMgr::RedisMgr() {
	auto& g_fg_Mgr = ConfigMgr::GetInstance();
	auto host = g_fg_Mgr["Redis"]["Host"];
	auto port = g_fg_Mgr["Redis"]["Port"];
	auto pwd = g_fg_Mgr["Redis"]["Passwd"];
...
//	if (_connect == nullptr) return false;
//	if (_connect->err) {
