...
ApplyFriend::ApplyFriend(QWidget* parent)
  ...
  this->setModal(true);
  ui->name_ed->setPlaceholderText(tr("rEin"));
  ui->lb_ed->setPlaceholderText("搜索、添加标签");
  ui->back_ed->setPlaceholderText("2ein");
...
void ApplyFriend::addLabel(QString name) {
  ...
  if (ui->gridWidget->height() < _label_point.y() + tmplabel->height() + 2) {
    ui->gridWidget->setFixedHeight(_label_point.y() + tmplabel->height() * 2 +
                                   2);
  }
}
...
// 点击标已有签添加或删除新联系人的标签
void ApplyFriend::SlotChangeFriendLabelByTip(QString lbtext,
                                             ClickLbState state) {
...
void ApplyFriend::SlotApplySure() {
  ...
  // 发送tcp请求给chat server
  emit TCPMgr::GetInstance() -> sign_send_data(ReqId::ID_ADD_FRIEND_REQ,
                                               jsonString);
  this->hide();
  deleteLater();
}
