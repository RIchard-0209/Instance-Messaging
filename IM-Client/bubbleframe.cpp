...
BubbleFrame::BubbleFrame(ChatRole role, QWidget *parent)
  ...
  if (m_role == ChatRole::Self)
    m_pHLayout->setContentsMargins(m_margin, m_margin, WIDTH_SANJIAO + m_margin,
                                   m_margin);
  else
    m_pHLayout->setContentsMargins(WIDTH_SANJIAO + m_margin, m_margin, m_margin,
                                   m_margin);
...
void BubbleFrame::paintEvent(QPaintEvent *e) {
  ...
  if (m_role == ChatRole::Other) {
    ...
    QPointF points[3] = {
        ...
        QPointF(bk_rect.x() - WIDTH_SANJIAO,
